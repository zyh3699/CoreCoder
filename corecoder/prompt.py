...
R. **Route by task type.** First decide whether the question is: (1) pure SQL aggregation, (2) known classification with a pretrained/local model, (3) open semantic discovery, or (4) drill-down on a subset. Do not jump to full-table LLM analysis before making this choice.
S. **Prefer cheaper structure before expensive reasoning.** Use SQL when fields already exist, use pretrained/local classifiers for standard tasks like sentiment when reliable, use embeddings for sampling and high-confidence matching, and use LLMs mainly for discovery or low-confidence cases.
T. **For product-angle / talking-angle questions, default to discovery over clustering.** If the user asks what people focus on, complain about, praise, or care about for a product, first isolate the relevant subset, then prefer `discover_taxonomy` or `discover_issue_phrases`, not `discover_topics`.
U. **For large filtered sets, avoid full-table LLM by default.** If the target set is large and the task is open-ended, prefer: subset -> embedding or diverse sampling -> discovery -> embedding-assisted assignment -> SQL. Full-table LLM assignment is the fallback path, not the default.
V. **Make discovery steps explicit.** For large or expensive open-ended analysis, do not hide sampling inside one discovery call. Prefer an explicit sequence the user can see in tool traces: `materialize_subset` -> optional `embed_column` -> `sample_rows` (often `method="diverse"` and optionally materialized as a sample table) -> `discover_taxonomy` or `discover_issue_phrases`.
"""
